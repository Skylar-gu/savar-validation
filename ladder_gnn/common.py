"""GNN PORT of the SAVAR SAE->PCMCI+ oracle-ablation ladder (MeshGNN, hetdynamics_eqvar).
Differences from ../savar_sae_pcmci/common.py: dataset, tau_max=6, activations at STRIDE 1.
"""
...
OUT = ROOT / "results" / "ladder_gnn"       # ladder outputs (filenames unchanged)
...
ACTS_CACHE = OUT / "activations_stride1_all.npy"
Z_CACHE = OUT / "Z_stride1_all.npy"
OUT.mkdir(parents=True, exist_ok=True)

TAU_MAX, ALPHA, PC_ALPHA = 6, 0.05, 0.2   # eqvar true lags reach 6
PROTOCOL = os.environ.get("LADDER_PROTOCOL", "ladder")   # "ladder" | "plus"
...
def pcmci_one(series):
    ...
    if PROTOCOL == "plus":      # Block G / aggregation_consistency.py protocol (0.853 anchor)
        res = pc.run_pcmciplus(tau_min=0, tau_max=TAU_MAX, pc_alpha=0.05)
        g = res["graph"]; N = g.shape[0]
        det = {(c, e, tau) for c in range(N) for e in range(N) if c != e
               for tau in range(1, TAU_MAX + 1) if g[c, e, tau] == "-->"}
        return det, res["p_matrix"], res["val_matrix"]
...
def load_gt():
    d = np.load(DATA_DIR / "realisation_000.npz")
