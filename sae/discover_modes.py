...
def pcmci_one(args):
    ...
    _ci = os.environ.get("E1_CITEST", "robustparcorr").lower()
    if _ci == "parcorr":
...
def score_candidate(What, mapping, series_fn, tag, save_sets=False):
    ...
    with ProcessPoolExecutor(max_workers=int(os.environ.get("MAX_WORKERS", min(30, max(1, (os.cpu_count() or 6) - 2)))), initializer=_worker_init) as ex:
