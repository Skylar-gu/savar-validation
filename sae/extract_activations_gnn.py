...
model.load_state_dict(ckpt["model_state"])   # strict: default gcn/emb0 matches ckpt
model.eval()
print(f"Loaded MeshGNN  val RMSE = {ckpt.get('val_rmse', float('nan')):.4f}  "
      f"device = {DEVICE}  reading layers[{LAYER}] output")

# ── register hook on the chosen MP layer (output H: (B, L, hidden)) ────────────
