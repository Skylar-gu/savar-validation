"""
Generate SAVAR realisations on a FINE temporal cadence with HETEROGENEOUS
coupling timescales, NONLINEAR dynamics, and NON-GAUSSIAN innovations.

Purpose (combines three design-note corrections + the subsample experiment)
---------------------------------------------------------------------------
1. NONLINEARITY (note §2)   — saturating AR + bilinear advective coupling in
   mode space, ported from generate_nonlinear.py. A linear-Gaussian process
   makes the optimal forecaster linear; real dynamics are not.
2. NON-GAUSSIANITY (note §5) — mode innovations are skewed/heavy-tailed,
   standardized to unit variance. This (a) stresses ParCorr's Gaussian
   calibration and (b) per the LiNGAM result makes contemporaneous direction
   identifiable from higher moments — exactly the regime where the
   now-aliased τ=0 edges (below) could in principle be oriented.
3. FINE CADENCE + HETEROGENEOUS LAGS — the ground-truth graph is defined at a
   fine cadence with each cross-edge assigned a DIFFERENT fine lag ℓ (in fine
   steps). Different modes therefore couple on different timescales. When the
   series is later subsampled at stride s (run_pcmci_subsample.py), an edge of
   fine lag ℓ maps to coarse lag  τ = ℓ // s,  so couplings faster than the
   sampling interval (ℓ < s) ALIAS INTO τ=0 (contemporaneous). Sweeping s moves
   the fast/slow boundary, which is why PCMCI+ (contemporaneous + lagged) is
   required downstream — plain PCMCI with tau_min=1 would miss every aliased
   edge.

The edge SET is identical to instantiate_model.py (controlled comparison); only
each edge's lag is reassigned to spread the timescales, and the noise/dynamics
become non-Gaussian/nonlinear. Innovations are standardized to unit variance so
the VAR stationarity condition (which depends only on the transition matrices,
not the noise law) is preserved — the note's "one-line swap".
...
# Same edge set as instantiate_model.py; each CROSS edge reassigned a fine lag.
...
links_coeffs = {
    0: [((0, -1),  0.45), ((2, -3),  0.22)],                      # auto + X2→X0 (ℓ3)
    1: [((1, -1),  0.50), ((0, -1),  0.35)],                      # auto + X0→X1 (ℓ1)
    2: [((2, -1),  0.35), ((1, -1),  0.40)],                      # auto + X1→X2 (ℓ1)
    3: [((3, -1),  0.55), ((0, -1),  0.30), ((2, -2), -0.30)],    # X0→X3 (ℓ1), X2→X3 (ℓ2)
    4: [((4, -1),  0.40), ((1, -3),  0.25)],                      # auto + X1→X4 (ℓ3)
    5: [((5, -1),  0.30), ((4, -2),  0.35), ((0, -4), -0.20)],    # X4→X5 (ℓ2), X0→X5 (ℓ4)
    6: [((6, -1),  0.50), ((3, -2),  0.30), ((5, -6),  0.25)],    # X3→X6 (ℓ2), X5→X6 (ℓ6)
    7: [((7, -1),  0.45), ((6, -4),  0.20), ((3, -6), -0.15)],    # X6→X7 (ℓ4), X3→X7 (ℓ6)
}
...
OUT_DIR = os.environ.get("FC_OUT_DIR", os.path.join("data", f"realisations_finecadence{_SUFFIX}"))
