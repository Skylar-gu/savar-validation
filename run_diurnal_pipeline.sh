...
STATUS=logs/diurnal/STATUS.txt
ts() { date '+%Y-%m-%d %H:%M:%S'; }
: > "$STATUS"
...
( if python3 pcmci/run_pcmci_diurnal.py > logs/diurnal/02_pcmci.log 2>&1; then
    echo "[$(ts)] OK    02 pcmci" >> "$STATUS"
  else
    echo "[$(ts)] FAIL  02 pcmci (see logs/diurnal/02_pcmci.log)" >> "$STATUS"
  fi ) &
PCMCI_PID=$!
...
python3 sae/extract_activations.py --diurnal > logs/diurnal/03_extract.log 2>&1
EX_RC=$?
...
# 4. Per-mode SAEs (GPU)
run "04 sae_train" logs/diurnal/04_sae_train.log python3 sae/train_sae_per_mode.py --diurnal
# 5. SAE alignment / monosemanticity
run "05 sae_eval"  logs/diurnal/05_sae_eval.log  python3 sae/eval_sae_per_mode.py  --diurnal
# 6. Cycle vs PC0 collapse analysis
run "06 cycle_pc0" logs/diurnal/06_cycle_pc0.log python3 sae/analyze_cycle_pc0.py
...
grep -E "Best val RMSE" logs/diurnal/01_cnn_train.log 2>/dev/null | tail -1 | tee -a "$STATUS" || true
grep -E "SUMMARY" logs/diurnal/02_pcmci.log 2>/dev/null | tail -1 | tee -a "$STATUS" || true
grep -E "Aligned|Strong" logs/diurnal/05_sae_eval.log 2>/dev/null | tee -a "$STATUS" || true
grep -E "Mean PC0 var" logs/diurnal/06_cycle_pc0.log 2>/dev/null | tee -a "$STATUS" || true
