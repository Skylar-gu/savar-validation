...
| R3 **multivariate** | 2–3 coupled observed channels per node, cross-channel edges in Φ | coupled atmospheric fields | Ŵ must become (channel × space); cross-channel edges test PCMCI+ conditioning | ~3 d → **DONE 2026-07-08: PX Spearman +0.943 ≥ 0.8 PASS. Pixel discovery clean (vmax_pix F1 0.755, 16/16); acts arm degenerate (km_act 0.000 — one hidden per spatial node can't separate co-located channels, documented). Dyn live. Selector works via pixel-int × dyn. Homogeneous-resolution pool (all ~16 modes) — clarifies R4's miss as pool-heterogeneity not scale.** |
| R4 **scale** | N = 24 modes, larger grid, unknown-N discovery | realistic mode count; N unknown a priori | N̂ estimation (Stage 1) and PCMCI+ parent search; Ising/Leiden favored over varimax here | ~3 d → **DONE 2026-07-08 (SPLIT): Leiden scales, varimax collapses — Leiden N̂=24, F1 0.562 (matched 23/24) vs vmax_act F1 0.032 / km_act,dmd_act 0.000; BUT pool-crossed selector MISSES at scale — PX Spearman +0.357 < 0.8 (dyn LIVE so real miss not inapplicable). Caveats: heterogeneous candidate resolution (12 vs 24), no corrupted anchors, N=7 → flag for follow-up (scale-appropriate anchors), not clean refutation. E2 flips positive (+0.559).** |
| R5 **rollout training** | fine-tune the forecaster on 4–8-step rollout (GraphCast's curriculum) | autoregressive training | does trained-on-rollout change F5's damping and E3's channel? Also widens per-mode skill spread (next_steps D3.1) | ~1 d → **DONE 2026-07-08: PX Spearman +0.934 ≥ 0.8 (PASS, ≈ R1 +0.946); rollout shifted internals (vmax_act truth-F1 0.485→0.556, coherences spread) but selector robust; 1-step RMSE preserved 1.2955→1.2996, 8-step improved 1.4756→1.4632; dyn live, E2 screen-not-ranker replicates. Built on R1 overlap world (isolates rollout effect).** |
